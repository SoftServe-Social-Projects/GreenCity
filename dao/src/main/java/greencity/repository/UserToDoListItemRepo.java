...
public interface UserToDoListItemRepo extends JpaRepository<UserToDoListItem, Long> {
     ...
     *
     * @param targetId id of needed goal
     * @param habitAssignId      id of needed habit assign
     */
    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "DELETE FROM user_to_do_list utdl "
        + "WHERE utdl.target_id =:targetId AND utdl.habit_assign_id =:habitAssignId ")
    void deleteByToDoListItemIdAndHabitAssignId(Long targetId, Long habitAssignId);
    ...
    @Query(nativeQuery = true,
        value = "SELECT target_id FROM user_to_do_list WHERE habit_assign_id = :id")
    ...
    @Query(nativeQuery = true, value = """
        ...
        where ha.user_id = :userId and utdl.target_id = :itemId""")
     ...
     * @author Anton Bondar
     */
    @Query(value = "SELECT utdli FROM UserToDoListItem utdli JOIN HabitAssign ha ON ha.id = utdli.habitAssign.id "
        + "WHERE ha.user.id =:userId AND utdli.id =:userToDoListItemId")
