...
public interface NotificationRepo extends CustomNotificationRepo, JpaRepository<Notification, Long>,
    ...
    /**
     * Finds a {@link Notification} based on a unique combination of the target user's ID, the notification type,
     * and an identifier that either matches the {@code secondMessageId} (if present) or the {@code targetId}.
     * <p>
     * This query is designed to find a unique notification by the following criteria:
     * @param targetUserId user, that should receive Notification
     * @param notificationType type of Notification
     * @param identifier identifier: either {@code secondMessageId} or {@code targetId}
    ...
    /**
     * Finds an unviewed {@link Notification} by target user's ID, notification type, target ID, and second message ID.
     * <p>
     * Returns a notification that matches the provided {@code targetUserId}, {@code notificationType},
     * {@code targetId}, and {@code secondMessageId}, with the {@code viewed} flag set to {@code false}.
     * </p>
     *
     ...
     * @return an {@link Optional} containing the matching notification, or empty if not found.
     */
